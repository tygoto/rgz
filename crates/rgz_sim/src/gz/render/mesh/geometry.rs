...
pub(crate) fn geometry(geometry: Option<&msgs::Geometry>) -> Option<Mesh> {
    ...
    match geometry_type {
        msgs::geometry::Type::Box => {
            if let Some(msgs::BoxGeom {
                size: Some(size),
                header: _,
            }) = geometry.r#box.as_ref()
            {
                let size = utils::size3(&size);
                return Some(Mesh::from(shape::Box::new(size.x, size.y, size.z)));
            }
        }
        ...
        msgs::geometry::Type::Image => if let Some(image_geom) = geometry.image.as_ref() {},
        msgs::geometry::Type::Heightmap => {
            if let Some(heightmap_geom) = geometry.heightmap.as_ref() {}
        }
        msgs::geometry::Type::Mesh => if let Some(mesh_geom) = geometry.mesh.as_ref() {},
        msgs::geometry::Type::TriangleFan => {}
        msgs::geometry::Type::LineStrip => {}
        msgs::geometry::Type::Polyline => for v in &geometry.polyline {},
        msgs::geometry::Type::Cone => if let Some(cone_geom) = geometry.cone.as_ref() {},
        ...
        msgs::geometry::Type::Ellipsoid => {
            if let Some(msgs::EllipsoidGeom {
                radii: Some(radii),
                header: _,
            }) = geometry.ellipsoid.as_ref()
            {
