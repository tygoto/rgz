...
impl <T> DispatcherStore<T> where T: Dispatcher {
    pub fn new() -> Self {
        DispatcherStore {
            topics: HashMap::new(),
        }
    ...
    pub fn register(&mut self, dispatcher: T) -> Result<()> {
        ...
        let nodes =
            self.topics.entry(topic).or_insert(HashMap::new());
        ...
        nodes.entry(node_uuid).or_insert(HashMap::new())
    ...
    pub fn find(&mut self, topic: &str, request_type: Option<&str>, response_type: Option<&str>)
                       -> Option<&mut T>
    {
        self.topics.get_mut(topic).and_then(|nodes| {
            nodes.iter_mut().flat_map(|(_, n)| n.values_mut())
    ...
    pub fn get_for_topic(&mut self, topic: &str) -> Option<Vec<&mut T>> {
        self.topics.get_mut(topic).and_then(|nodes| {
            Some(nodes.values_mut()
                .flat_map(|dispatchers| dispatchers.values_mut())
                .collect())
        })
    }

    pub fn filter(&mut self, topic: &str, request_type: Option<&str>, response_type: Option<&str>)
                       -> Option<Vec<&mut T>>
    {
        self.topics.get_mut(topic).and_then(|nodes| {
           Some(nodes.iter_mut().flat_map(|(_, n)| n.values_mut())
    ...
    pub(crate) fn print(&self) {
        ...
        self.topics.iter().for_each(|(topic, nodes)| {
            ...
            nodes.iter().for_each(|(node_uuid, dispatchers)| {
                ...
                dispatchers.iter().for_each(|(dispatcher_uuid, dispatcher)| {
